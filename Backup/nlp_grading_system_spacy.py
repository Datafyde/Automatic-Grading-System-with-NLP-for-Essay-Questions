...
# Function to validate the uploaded CSV file
def validate_csv_spacy(file, expected_columns):
    try:
...
def grade_questions_spacy(key_df, response_df):
    """
...
3. **Grading System with Spacy**:File uploads, grading, and results with Spacy
4. **User Guide Page**: Instructions on how to use the system.
5. **About Page**: Credits and acknowledgements.
...
def grading_system_spacy_page():
    ...
    key_file = st.file_uploader('"QuestionID", "Correct_Answer","Question_Type","Marks_Obtainable"', type=["csv"], key="key_file")  # File uploader for the answer key
    ...
    response_file = st.file_uploader('"StudentID", "QuestionID", "Student_Answer","Question_Type"', type=["csv"], key="response_file")  # File uploader for student responses
    ...
    if key_file:  # If a file has been uploaded
        key_df, key_error = validate_csv_spacy(key_file, expected_key_columns)  # Validate the file structure
    ...
    if response_file:  # If a file has been uploaded
        response_df, response_error = validate_csv_spacy(response_file, expected_response_columns)  # Validate the file structure
    ...
    if key_file and response_file and key_df is not None and response_df is not None:
        ...
        if st.button("Show Results", type="primary"):
            ...
            scores_df = grade_questions_spacy(key_df, response_df)

            if scores_df is not None:
                st.dataframe(scores_df)  # Display the grading results in a table
                file = scores_df.set_index('StudentID')
                file = file.to_csv().encode("utf-8")
            else:
                st.error("Grading failed. Please check the input files and try again.")
            st.bar_chart(data=scores_df, x='StudentID', y='Score', horizontal=True, height=300)
            col1, col2, col3 = st.columns(3)

            col1.download_button("Download final result", file_name="final.csv", data = file, mime="text/csv", type='primary')
            
            if col3.button("Refresh", type='primary'):
                grading_system_spacy_page()
...
if st.sidebar.button("Grading System with Spacy", type="primary"):
    st.session_state.page = "Grading System with Spacy"
...
elif st.session_state.page == "Grading System with Spacy":
    grading_system_spacy_page()
